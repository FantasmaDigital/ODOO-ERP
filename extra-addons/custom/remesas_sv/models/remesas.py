...
class ListAsientos(models.Model):
  ...
  name = fields.Char('Nombre',
                     related='asiento_contable_id.name')
...
class Remesa_Sv(models.Model):
    ...
    total = fields.Float('Total Remesa',
                         digits='Account',
                         required=True, readonly=False, help="Total depositado al banco")

    referencia = fields.Char('Referencia Banco', required=True, readonly=False,
                             help="Ingrese el numero de voucher correspondiente de la remesa")

    banco_id = fields.Many2one('res.bank', 'Banco', ondelete='restrict',
    ...
    def contabilizar_remesa(self):
        if not self.company_id.transfer_account_id:
            raise exceptions.UserError(
                _('Cuenta de Transferencia No Establecida. Es requerido Establecer una cuenta para Transferencias Bancarias'))
        if not self.cuenta_id:
            raise exceptions.UserError(
                _('Cuenta No Selecccionada! Por Favor seleccionar una cuenta bancaria para poder registrar la remesa'))
        if not self.cuenta_id.default_debit_account_id or not self.origen_id.default_credit_account_id:
            raise exceptions.UserError(
                _('Cuenta No Asignada! Por Favor seleccionar una cuenta bancaria configurada correctamente para poder registrar la remesa'))
        if not self.total:
            raise exceptions.UserError(
                _('Total No Asignado! Por Favor Ingrese un total valido para poder registrar la remesa'))
        if not self.referencia:
            raise exceptions.UserError(
                _('Referencia No Asignada! Por Favor Ingrese una referencia valida para poder registrar la remesa'))
