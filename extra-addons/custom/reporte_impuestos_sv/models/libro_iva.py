...
class LibroIva(models.Model):
  ...
  name = fields.Char('Nombre', copy=False, default='Libro de Iva', tracking=True)
  active=fields.Boolean('Activo', default=True)
  fecha = fields.Date(string=_('Fecha'), 
      index=True, help=_("Fecha de realizado el informe"), copy=False, default=time.strftime('%Y-%m-%d'), tracking=True)
  
  company_id = fields.Many2one('res.company', string=_('Compañia'), change_default=True,
      default=lambda self: self.env.user.company_id.id)
  
  company_currency_id = fields.Many2one('res.currency', related='company_id.currency_id', string="Company Currency", readonly=True)
  
  nrc = fields.Char(string=_("NRC"), readonly=True,
      help=_("Numero de registro contribuyente"), default= lambda self: self.env.user.company_id.nrc)
  
  nit = fields.Char(string=_("NIT"), readonly=True,
      help=_("Numero de Identificacion Tributario"), default= lambda self: self.env.user.company_id.nit)
  ...
  mes = fields.Selection([
    ...
    ('11','Noviembre'),
    ('12','Diciembre'),
  ], string=_("Mes"), index=True, default=time.strftime("%m"), copy=False, required=True, tracking=True)
  ...
  responsable_id = fields.Many2one('res.users', _('Responsable'), required=True, readonly=True,
        help=_("Seleccione la persona que validara el libro"), tracking=True)
  
  usuario_id = fields.Many2one('res.users', string=_('Usuario'),
      readonly=True, default=lambda self: self.env.user)
  
  
  type = fields.Selection([('fcf','Libro Consumidor Final'),('ccf','Libro Credito Fiscal'), ('compras','Libro Compras')],string=None, default=lambda self: self._context.get('type', 'fcf'),)
  
  #campos que contienen el detalle y resumen del reporte.
  
  libro_line_compras =  fields.One2many('libro.line', 'libro_iva_id', 'Lineas Libro Compras', readonly=True)
  libro_line_ccf =  fields.One2many('libro.line', 'libro_iva_id', 'Lineas Libro CCF', readonly=True)
  libro_line_fcf =  fields.One2many('libro.line', 'libro_iva_id', 'Lineas Libro FCF', readonly=True)
  resumen_line_fcf =  fields.One2many('resumen.line', 'libro_iva_id', 'Lineas Resumen FCF', readonly=True)
  resumen_line_ccf =  fields.One2many('resumen.line', 'libro_iva_id', 'Lineas Resumen CCF', readonly=True)
  resumen_line_compras =  fields.One2many('resumen.line', 'libro_iva_id', 'Lineas Resumen Compras', readonly=True)
  ...
  def datos_iva_compra(self):
    ...
    for i in range(dia[1]):
      ...
      invoice_list = invoice_obj.search([
                  ...
                  ('state', '=', 'open'),
                  ('type', '=','in_invoice'),
                  ('journal_id.type_report', '=', 'compras')], order='number, date_invoice')
      ...
      for inv_id in invoice_list:
        ...
        if inv_id.state_refund == 'no_refund':
          for tax_id in inv_id.tax_line_ids:
            if tax_id.tax_id.type_tax == 'iva_compra':
            ...
            if tax_id.tax_id.type_tax == 'percepcion1' or tax_id.tax_id.type_tax == 'percepcion2':
              percepcion += abs(tax_id.amount)
            if tax_id.tax_id.type_tax == 'retencion':
      ...
      for tax in tax_ids:
        ...
        for m in move_list:
          ...
          percepcion = m.balance if tax.type_tax == 'percepcion1' or tax.type_tax == 'percepcion2' else 0
          self.env['libro.line'].create({
            'libro_iva_id': self.id,
            'correlativo': correlativo,
            'fecha_doc': fecha,
  ...
  def resumen_compras(self):
    ...
    for l in self.libro_line_compras:
      ...
      ##print ventas_exentas, ventas_gravadas, exportaciones, retenciones, debito_fiscal, totales
  ...
  def datos_iva_ccf(self):
    ...
    for i in range(dia[1]):
      ...
      invoice_list = invoice_obj.search([
                  ...
                  ('state', '=', 'open'),
                  ('type', '=','out_invoice'),
                  ('journal_id.type_report', '=', 'ccf')], order='number, date_invoice')
      print(invoice_list)
      for inv_id in invoice_list:
        debito_fiscal = 0
        ventas_gravadas = 0
        ...
        if inv_id.state_refund == 'no_refund':
          for tax_id in inv_id.tax_line_ids:
            if tax_id.tax_id.type_tax == 'iva_venta':
          ...
          if datetime.strptime(inv_id.inv_refund_id.date_invoice, '%Y-%m-%d').month == fecha.month:
            refund_list.append(inv_id.inv_refund_id.id)
            self.env['libro.line'].create({
              'libro_iva_id': self.id,
              'correlativo': correlativo,
      ...
      print(refund_old)
      for r_id in refund_old:
        numeracion = func.numeracion(r_id.number)
        prefijo = numeracion.get('pre')
        longitud = numeracion.get('longitud')
        numero = int(numeracion.get('num'))
        name = r_id.partner_id.name
        nrc = r_id.partner_id.nrc
        numero = r_id.number
        debito_fiscal = 0
        ventas_gravadas = 0
        ventas_exentas = 0
  ...
  def datos_iva_fcf(self):
    ...
    for i in range(dia[1]):
      ...
      invoice_list = invoice_obj.search([
                  ...
                  ('state', '=', 'open'),
                  ('type', '=','out_invoice'),
                  ('journal_id.type_report', '=', 'fcf')], order='number, date_invoice')
      ...
      for inv_id in invoice_list:
        ...
        if inv_id.state_refund == 'no_refund':
          ...
          if num_inicial == 0:
            ...
            if prefijo_ant != prefijo or (num_anterior + 1) != numero:
              #Guardar
              print(prefijo_ant,prefijo,num_anterior,numero,'*******************')
              self.env['libro.line'].create({
                  'libro_iva_id': self.id,
                  'dia': i,
          ...
          for tax_id in inv_id.tax_line_ids:
            ...
            if tax_id.tax_id.type_tax == 'retencion':
              retenciones += abs(tax_id.amount)
            if tax_id.tax_id.type_tax == 'exportacion':
          ...
          if datetime.strptime(inv_id.inv_refund_id.date_invoice, '%Y-%m-%d').month == fecha.month:
            ...
            #creamos un salto guardamos los datos de la retificativa
      ...
      refund_old = self.env['account.invoice'].search([
        ...
        ('date_invoice','=',fecha),
        ('type','=',"out_refund"),
        ('journal_id.type_report','in',['anu','dev']),
